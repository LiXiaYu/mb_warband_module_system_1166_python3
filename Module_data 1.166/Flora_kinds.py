...
def save_fauna_kinds():
  ...
  file.write("{0:d}\n".format(len(fauna_kinds)))
  for fauna_kind in fauna_kinds:
    meshes_list = fauna_kind[2]
    file.write("{0:s} {1:d} {2:d}\n".format(fauna_kind[0], (dword_mask & fauna_kind[1]), len(meshes_list)))
    for m in meshes_list:
      file.write(" {0:s} ".format(m[0]))
      if (len(m) > 1):
        file.write(" {0:s}\n".format(m[1]))
      else:
        file.write(" 0\n")
      if ( fauna_kind[1] & (fkf_tree|fkf_speedtree) ):  #if this fails make sure that you have entered the alternative tree definition (NOT FUNCTIONAL in Warband)
        ...
        file.write(" {0:s} {1:s}\n".format(speedtree_alternative[0], speedtree_alternative[1]))
    if ( fauna_kind[1] & fkf_has_colony_props ):
      file.write(" {0:s} {1:s}\n".format(fauna_kind[3], fauna_kind[4]))
  file.close()
...
def save_python_header():
  ...
  for i_fauna_kind in range(len(fauna_kinds)):
    file.write("{0:s}_1 = 0x".format(fauna_kinds[i_fauna_kind][0]))
    file.write("{0:x}\n".format(fauna_mask | two_to_pow(i_fauna_kind)))
    file.write("{0:s}_2 = 0x".format(fauna_kinds[i_fauna_kind][0]))
    file.write("{0:x}\n".format(fauna_mask | ((low_fauna_mask|two_to_pow(i_fauna_kind)) << 64)))
    file.write("{0:s}_3 = 0x".format(fauna_kinds[i_fauna_kind][0]))
    file.write("{0:x}\n".format(fauna_mask | ((low_fauna_mask|two_to_pow(i_fauna_kind)) << 64) | two_to_pow(i_fauna_kind)))
  file.close()

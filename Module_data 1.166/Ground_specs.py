...
def write_vec(file,vec):
  file.write(" {0:f} {1:f} {2:f} ".format(vec))
  
def save_ground_specs():
  file = open("./ground_specs.txt","w")
  for ground_spec in ground_specs:
    file.write(" {0:s} {1:d} {2:s} {3:f} {4:s}".format(ground_spec[0],ground_spec[1],ground_spec[2],ground_spec[3],ground_spec[4]))
    if (ground_spec[1] & gtf_has_color):
      file.write(" {0:f} {1:f} {2:f}".format(ground_spec[5]))
    file.write("\n")
  file.close()
...
def save_c_header():
  ...
  for ground_spec in ground_specs:
    file.write("  ground_{0:s},\n".format(ground_spec[0]))
  file.write("}Ground_spec_codes;\n")
  file.write("const int num_ground_specs = {0:d};\n".format(len(ground_specs)))
  file.write("\n\n")
  file.write("\n#endif\n")
  file.close()
...
def save_python_header():
  ...
  for ig in range(len(ground_specs)):
    ...
    file.write("ground_{0:s} = {1:d}\n".format(ground_spec[0], ig))
  file.write("\n\n")
  file.close()

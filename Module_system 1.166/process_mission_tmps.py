...
def save_triggers(file,template_name,triggers,variable_list,variable_uses,tag_uses,quick_strings):
  ...
  for i in range(len(triggers)):
    trigger = triggers[i]
    file.write("{0:f} {1:f} {2:f} ".format(trigger[trigger_check_pos],trigger[trigger_delay_pos],trigger[trigger_rearm_pos]))
...
def save_mission_template_group(file,entry):
  if (len(entry[5]) > 8):
    ...
    error()
  file.write("{0:d} {1:d} {2:d} {3:d} {4:d} {5:d}  ".format(entry[0],entry[1],entry[2],entry[3],entry[4], len(entry[5])))
  for item_override in entry[5]:
    add_tag_use(tag_uses,tag_item,item_override)
    file.write("{0:d} ".format(item_override))
  file.write("\n")
...
def save_mission_templates(variables,variable_uses,tag_uses,quick_strings):
  ...
  file.write(" {0:d}\n".format(len(mission_templates)))
  for mission_template in mission_templates:
    file.write("mst_{0:s} {1:s} {2:d} ".format(convert_to_identifier(mission_template[mission_template_name_pos]),convert_to_identifier(mission_template[mission_template_name_pos]),mission_template[mission_template_flags_pos]))
    file.write(" {0:d}\n".format(mission_template[mission_template_types_pos]))
    file.write("{0:s} \n".format(mission_template[mission_template_desc_pos].replace(" ","_")))
    file.write("\n{0:d} ".format(len(mission_template[mission_template_groups_pos])))
...
def save_python_header():
  ...
  for i_mission_template in range(len(mission_templates)):
    file.write("mst_{0:s} = {1:d}\n".format(mission_templates[i_mission_template][0],i_mission_template))
  file.close()

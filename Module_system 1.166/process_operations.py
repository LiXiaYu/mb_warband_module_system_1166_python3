...
def save_quick_strings(export_dir, quick_strings):
  ...
  file.write("{0:d}\n".format(len(quick_strings)))
  for i in range(len(quick_strings)):
    file.write("{0:s} {1:s}\n".format(quick_strings[i][0],replace_spaces(quick_strings[i][1])))
  file.close()
...
def save_variables(export_dir,variables_list,variable_uses):
  ...
  for i in range(len(variables_list)):
    file.write("{0:s}\n".format(variables_list[i]))
  file.close()
  file = open(export_dir + "variable_uses.txt","w")
  for i in range(len(variables_list)):
    file.write("{0:d}\n".format(variable_uses[i]))
  file.close()
...
def save_tag_uses(export_dir,tag_uses):
  ...
  for i in range(len(tag_uses)):
    for j in range(len(tag_uses[i])):
      file.write("{0:d} {0:d} {0:d};".format((i, j, tag_uses[i][j])))
    file.write("\n")
  file.close()
...
def save_statement(ofile,opcode,no_variables,statement,variable_list,variable_uses,local_vars_list,local_var_uses,tag_uses,quick_strings):
  if no_variables == 0:
    ...
    lenstatement = 0
  ofile.write("{0:d} {1:d} ".format(opcode, lenstatement))
  for i in range(lenstatement):
    ...
    ofile.write("{0:d} ".format(operand))

def compile_global_vars_in_statement(statement,variable_list, variable_uses):
  opcode = 0
  if (((type(statement) is list)==False) and ((type(statement) is tuple)==False)):
    opcode = statement
  else:
    opcode = statement[0]
...
def save_statement_block(ofile,statement_name,can_fail_statement,statement_block,variable_list, variable_uses,tag_uses,quick_strings):
  local_vars = []
  local_var_uses = []
  ofile.write(" {0:d} ".format(len(statement_block)))
...
def save_simple_triggers(ofile,triggers,variable_list, variable_uses,tag_uses,quick_strings):
  ...
  for trigger in triggers:
    ofile.write("{0:f} ".format(trigger[0]))

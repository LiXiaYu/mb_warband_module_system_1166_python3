...
def save_psys_keys(ofile, keys1, keys2):
    ofile.write("{0:f} {1:f}   {2:f} {3:f}\n".format(keys1[0], keys1[1], keys2[0], keys2[1]))

def save_particle_systems():
  ofile = open(export_dir + "particle_systems.txt","w")
  ofile.write("particle_systemsfile version 1\n")
  ofile.write("{0:d}\n".format(len(particle_systems)))
  for psys in particle_systems:
    ofile.write("psys_{0:s} {1:d} {2:s}  ".format(psys[0], psys[1], psys[2]))
    ofile.write("{0:d} {1:f} {2:f} {3:f} {4:f} {5:f} \n".format(psys[num_particles_pos], psys[life_pos], psys[damping_pos], psys[gravity_pos], psys[turb_size_pos], psys[turb_wt_pos]))
    ...
    ofile.write("{0:f} {1:f} {2:f}   ".format(psys[emit_box_size_pos][0],psys[emit_box_size_pos][1],psys[emit_box_size_pos][2]))
    ofile.write("{0:f} {1:f} {2:f}   ".format(psys[emit_velocity_pos][0],psys[emit_velocity_pos][1],psys[emit_velocity_pos][2]))
    ofile.write("{0:f} \n".format(psys[emit_rndmness_pos]))
    if (len(psys) >= (angular_speed_pos + 1)):
      ofile.write("{0:f} ".format(psys[angular_speed_pos]))
    else:
      ofile.write("0.0 ")
    if (len(psys) >= (angular_damping_pos + 1)):
      ofile.write("{0:f} ".format(psys[angular_damping_pos]))
    else:
      ofile.write("0.0 ")
    ofile.write("\n")
  ofile.close()
...
def save_python_header():
  ...
  for i_particle_system in range(len(particle_systems)):
    ofile.write("psys_{0:s} = {1:d}\n".format(particle_systems[i_particle_system][0],i_particle_system))
  ofile.close()

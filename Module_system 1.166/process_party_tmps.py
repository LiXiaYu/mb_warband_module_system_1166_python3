...
#    add_tag_use(tag_uses,tag_troop,troop[0])
    file.write("{0:d} {1:d} {2:d} ".format(troop[0],troop[1],troop[2]))
    if (len(troop) > 3):
      file.write("{0:d} ".format(troop[3]))
    else:
      file.write("0 ")
  else:
    file.write("-1 ")
...
def save_party_templates():
  ...
  file.write("{0:d}\n".format(len(party_templates)))
  for party_template in party_templates:
#    add_tag_use(tag_uses,tag_faction,party_template[4])
    file.write("pt_{0:s} {1:s} {2:d} {3:d} {4:d} {5:d} ".format(convert_to_identifier(party_template[0]),replace_spaces(party_template[1]),party_template[2],party_template[3], party_template[4], party_template[5]))
...
def save_python_header():
  ...
  for i_party_template in range(len(party_templates)):
    file.write("pt_{0:s} = {1:d}\n".format(convert_to_identifier(party_templates[i_party_template][0]),i_party_template))
  file.close()
